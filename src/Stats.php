...
class Stats {
	 ...
	 */
	public function to_json( array $custom_data = array(), array $excludedFields = array() ): string {

		$actions = Chunk_DB::db()->get_chunks_by_status( group_name: $this->group_name );
		$actions = array_map(
			function ( Chunk $chunk ) use ( $excludedFields ) {
	 ...
	 */
	public function prepare_email_text( array $custom_data = array() ): string {
		...
		if ( ! empty( $custom_data ) ) {
			...
			foreach ( $custom_data as $key => $value ) {
				$email_text .= sprintf( __( '%1$s: %2$s', 'as-processor' ), ucfirst( $key ), ( is_array( $value ) ? json_encode( $value ) : $value ) ) . "\n";
			}
		}
	 ...
	 * @throws \Exception Unparsable Date.
	 */
	public function send_mail_report( string $to = '', string $subject = '', array $custom_data = array() ): void {

		$message   = $this->prepare_email_text( $custom_data );
		$mailarray = apply_filters(
			'asp/stats/remail_report',
			array(
				'to'      => $to ?: get_option( 'admin_email' ),
				'subject' => $subject ?: sprintf( __( '[%1$s] Report of sync group %2$s', 'sinnewerk' ), get_bloginfo( 'name' ), $this->group_name ),
				'message' => $message,
			)
		);
